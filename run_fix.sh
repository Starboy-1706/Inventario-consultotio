...
export function CurrencyProvider({ children }) {
  ...
  const syncOfficialRates = useCallback(async (notify = false) => {
    try {
      ...
      if (notify) toast.success(`Tasas: BCV Bs. ${updated.VES || rates.VES} | COP $${updated.COP || rates.COP}`)
    } catch {
      if (notify) toast.error('Error al sincronizar tasas')
    }
  }, [rates])
  ...
  return (
    <CurrencyContext.Provider value={{ rates, setRates, taxes, activeCur, setActiveCur, syncOfficialRates, loadData }}>
      {children}
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-xs text-slate-400">Resumen operativo del consultorio y ventas</p>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="card-box flex items-center justify-between">
          <div><p className="text-xs text-slate-400 font-medium">Pacientes</p><h3 className="text-2xl font-bold text-slate-800 mt-1">{stats.pacs}</h3></div>
        ...
        <div className="card-box flex items-center justify-between">
          <div><p className="text-xs text-slate-400 font-medium">Citas Hoy</p><h3 className="text-2xl font-bold text-slate-800 mt-1">{stats.citas}</h3></div>
        ...
        <div className="card-box flex items-center justify-between">
          <div><p className="text-xs text-slate-400 font-medium">Ventas</p><h3 className="text-xl font-bold text-slate-800 mt-1"><PriceBox usd={stats.ventasTotal} /></h3></div>
        ...
        <div className="card-box flex items-center justify-between">
          <div><p className="text-xs text-slate-400 font-medium">Stock Bajo</p><h3 className="text-2xl font-bold text-rose-600 mt-1">{stats.lowStock}</h3></div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="card-box space-y-3">
          <div className="flex justify-between items-center border-b pb-2">
            <h3 className="font-bold text-sm text-slate-800 flex items-center gap-2"><Calendar className="w-4 h-4 text-teal-600" /> Citas de Hoy</h3>
            <Link to="/citas" className="text-xs font-semibold text-teal-600 flex items-center gap-1 hover:underline">Ver agenda <ArrowUpRight className="w-3.5 h-3.5" /></Link>
          </div>
          <div className="space-y-2">
            {citasHoy.length === 0 ? <p className="text-xs text-slate-400 py-6 text-center">No hay citas programadas para hoy</p> :
            citasHoy.map(c => (
        ...
        <div className="card-box space-y-3">
          <div className="flex justify-between items-center border-b pb-2">
            <h3 className="font-bold text-sm text-slate-800 flex items-center gap-2"><Package className="w-4 h-4 text-rose-500" /> Stock Crítico</h3>
            <Link to="/inventario" className="text-xs font-semibold text-teal-600 flex items-center gap-1 hover:underline">Ir a inventario <ArrowUpRight className="w-3.5 h-3.5" /></Link>
          </div>
          <div className="space-y-2">
            {prodsBajos.length === 0 ? <p className="text-xs text-slate-400 py-6 text-center">Inventario en buen estado</p> :
            prodsBajos.map(p => (
...
# 22. Probar compilación limpia de producción
echo "📦 Instalando y compilando..."
npm install
npm run build

echo "🎉 ¡EXITOSO! El código se compiló perfectamente sin ningún error."
